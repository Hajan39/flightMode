...
- Logic games update: hint system ve vsech 3 logickych hrach (kazdy hint poheneny vlastnim solverem hry: Runway Jam = dalsi optimalni tah z BFS, Sky Pixels = jedna logicky vynucena bunka, Sun & Moon = jedna bunka reseni; pouziti hintu = strop 2 hvezdy), nove levely (runway-jam 12→18 expert tier, nonogram 10→15 obrazku 10×10, sun-moon 12→16 tezsich 8×8) — vse solver-verifikovane testy; lokalizovane a11y popisky (7 novych klicu × 12 jazyku)
- 3 nove logicke hry na rozvoj logickeho mysleni, kazdy level overeny solverem v unit testech (katalog 33 → 36 her): **Ucpana ranvej** (`runway-jam`, Rush Hour klon, 12 levelu, BFS solver overuje resitelnost + presny optimalni pocet tahu), **Nebeske pixely** (`nonogram`, picross, 10 puzzlu 5×5→10×10, line-solver dokazuje resitelnost cistou logikou bez hadani ⇒ unikatni reseni), **Slunce a mesic** (`sun-moon`, Takuzu/Binairo, 12 levelu 4×4→8×8, backtracking solver overuje presne jedno reseni). Vsechny: level-select s hvezdami (`levelStars`), 3 nove achievementy (Pozemni dispecer, Pixelovy umelec, Rovnovaha) vc. PGS mapy, preklady ve 12 jazycich.
- Nativni bridge pro push achievementu do Play Games: lokalni Android-only Expo modul (`modules/play-games/`, Kotlin nad Play Games v2) — sign-in, unlock/increment achievement, overlay. New-Arch-safe (bez neudrzovane komunitni lib), autolinkovany z `./modules` (overeno pres `expo-modules-autolinking`), napojeny pres `utils/playGames.ts`; JS strana no-op na iOS/web/Expo Go. Push achievementu jeste vyzaduje doplnit `CgkI…` Play ID do `data/playGamesAchievements.ts`. Kotlin/Gradle build overitelny az v nativnim buildu.
